...
if isReleaseAbove(10, 6):
    erasToTest = ['UL2017', 'UL2018']
else:
    erasToTest = ['2016', '2017', '2018']
...
for isAOD in [True, False]:
    if isAOD:
        treesToRun = ['tnpEleReco']
    else:
        treesToRun = ['tnpEleIDs', 'tnpPhoIDs', 'tnpEleTrig']

    for isMC in [False, True]:
        for era in erasToTest:
            if isAOD and not 'UL' in era:
                continue  # don't have specified AOD test files yet for rereco

            options = ['era=%s' % era, 'maxEvents=1000']
            options += ['isAOD=True', 'doRECO=True', 'doEleID=False', 'doPhoID=False',
                        'doTrigger=False'] if isAOD else ['doRECO=False', 'doEleID=True', 'doPhoID=True', 'doTrigger=True']
            ...
            outFile = 'rootfiles/TnPTree_%s_%s_%s.root' % (
                'mc' if isMC else 'data', era, 'AOD' if isAOD else 'miniAOD')
            shutil.move('TnPTree_%s.root' %
                        ('mc' if isMC else 'data'), outFile)

            for tree in treesToRun:

                report = 'log/report_%s_%s_%s_%s.log' % (
                    'mc' if isMC else 'data', era, 'AOD' if isAOD else 'miniAOD', tree)

                print('Testing for options %s and tree %s...' %
                      (' '.join(options), tree)),
                system('./compareTrees %s %s -s -d %s &> %s' % (outFile,
                       outFile.replace('.root', '_ref.root'), tree, report))
