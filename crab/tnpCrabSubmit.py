...
#
submitVersion = "2022-21-09"  # add some date here
# doL1matching = True
doL1matching = False
...
mainOutputDir = '/store/user/ftorresd/phys_egamma/tnpTuples/%s' % (
    submitVersion)
...
def submit(config, requestName, sample, era, json, extraParam=[]):
    ...
    config.Data.outLFNDirBase = '%s/%s/%s/' % (
        mainOutputDir, era, 'mc' if isMC else 'data')
    config.Data.splitting = 'FileBased' if isMC else 'LumiBased'
    config.Data.lumiMask = None if isMC else json
    config.Data.unitsPerJob = 5 if isMC else 25
    config.JobType.pyCfgParams = defaultArgs + \
        ['isMC=True' if isMC else 'isMC=False', 'era=%s' % era] + extraParam
...
def submitWrapper(requestName, sample, era, extraParam=[]):
    if doL1matching:
        ...
        for leg1Threshold, json in getLeg1ThresholdForDoubleEle(era.replace("UL", "").replace("preVFP", "").replace("postVFP", "")):
            ...
            p = Process(target=submit, args=(config, '%s_leg1Threshold%s' % (
                requestName, leg1Threshold), sample, era, json, extraParam + ['L1Threshold=%s' % leg1Threshold]))
            p.start()
            p.join()
    else:
        p = Process(target=submit, args=(config, requestName,
                    sample, era, getLumiMask(era), extraParam))
        p.start()
        p.join()
...
era = 'UL2016preVFP'
# submitWrapper(
#     'Run2016B', '/SingleElectron/Run2016B-ver2_HIPM_UL2016_MiniAODv2-v2/MINIAOD', era)
# submitWrapper(
#     'Run2016C', '/SingleElectron/Run2016C-HIPM_UL2016_MiniAODv2-v2/MINIAOD', era)
# submitWrapper(
#     'Run2016D', '/SingleElectron/Run2016D-HIPM_UL2016_MiniAODv2-v2/MINIAOD', era)
# submitWrapper(
#     'Run2016E', '/SingleElectron/Run2016E-HIPM_UL2016_MiniAODv2-v5/MINIAOD', era)
# submitWrapper(
#     'Run2016F', '/SingleElectron/Run2016F-HIPM_UL2016_MiniAODv2-v2/MINIAOD', era)
...
era = 'UL2016postVFP'
# submitWrapper(
#     'Run2016F', '/SingleElectron/Run2016F-UL2016_MiniAODv2-v2/MINIAOD', era)
# submitWrapper(
#     'Run2016G', '/SingleElectron/Run2016G-UL2016_MiniAODv2-v2/MINIAOD', era)
# submitWrapper(
#     'Run2016H', '/SingleElectron/Run2016H-UL2016_MiniAODv2-v2/MINIAOD', era)
...
submitWrapper('DY_LO',   '/DYJetsToLL_M-50_TuneCP5_13TeV-madgraphMLM-pythia8/RunIISummer20UL17MiniAODv2-106X_mc2017_realistic_v9_ext1-v1/MINIAODSIM', era)
