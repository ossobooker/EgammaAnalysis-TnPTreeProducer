...
registerOption('is80X',       False,
               'Compatibility to run on old 80X files')
...
registerOption('doRECO',      False,
               'Include tree for Reco SF (requires AOD)')
registerOption('calibEn',     False,
...
registerOption('HLTname',     'HLT',    'HLT process name (default HLT)',
               optionType=VarParsing.varType.string)  # HLTname was HLT2 in now outdated reHLT samples
registerOption('GT',          'auto',   'Global Tag to be used',
               optionType=VarParsing.varType.string)
registerOption('era',         '2018',   'Data-taking era: 2016, 2017, 2018, UL2017 or UL2018',
               optionType=VarParsing.varType.string)
registerOption('logLevel',    'INFO',   'Loglevel: could be DEBUG, INFO, WARNING, ERROR',
               optionType=VarParsing.varType.string)

registerOption('L1Threshold',  0,       'Threshold for L1 matched objects',
...
if varOptions.isAOD and varOptions.doEleID:
    log.warning(
        'AOD is not supported for doEleID, please consider using miniAOD')
...
options['OUTPUT_FILE_NAME'] = "TnPTree_%s.root" % (
    "mc" if options['isMC'] else "data")
...
if '2016' in options['era']:
    ...
    options['HLTFILTERSTOMEASURE'] = {"passHltEle27WPTightGsf":                           cms.vstring("hltEle27WPTightGsfTrackIsoFilter"),
                                      ...
                                      #  Fabian and Felipe edited from here ...
                                      # HLT_DoubleEle33_CaloIdL_GsfTrkIdVL
                                      "passDiEle33CaloIdLGsfTrkIdVLDPhiUnseeded":         cms.vstring("hltDiEle33CaloIdLGsfTrkIdVLDPhiUnseededFilter"),
                                      # HLT_Photon175
                                      "passEG175HE":                                      cms.vstring("hltEG175HEFilter"),
                                      # HLT_Ele115_CaloIdVT_GsfTrkIdT
                                      "passEle115CaloIdVTGsfTrkIdTGsfDphi":               cms.vstring("hltEle115CaloIdVTGsfTrkIdTGsfDphiFilter"),
    ...
    options['HLTFILTERSTOMEASURE'] = {"passHltEle32DoubleEGWPTightGsf":                   cms.vstring("hltEle32L1DoubleEGWPTightGsfTrackIsoFilter"),
                                      ...
                                      "passEle115CaloIdVTGsfTrkIdTGsfDphi":               cms.vstring("hltEle115CaloIdVTGsfTrkIdTGsfDphiFilter"),
                                      # HLT_DoubleEle33_CaloIdL_MW
                                      "passEG200HE":                                      cms.vstring("hltEG200HEFilter"),
                                      }
    ...
    options['HLTFILTERSTOMEASURE'] = {"passHltEle32WPTightGsf":                           cms.vstring("hltEle32WPTightGsfTrackIsoFilter"),
                                      ...
                                      "passDiEle25CaloIdLMWPMS2Unseeded":                 cms.vstring("hltDiEle25CaloIdLMWPMS2UnseededFilter"),
                                      # HLT_Ele115_CaloIdVT_GsfTrkIdT
                                      "passEle115CaloIdVTGsfTrkIdTGsfDphiFilter":          cms.vstring("hltEle115CaloIdVTGsfTrkIdTGsfDphiFilter"),
                                      # HLT_Photon200
                                      "passEG200HEFilter":                                 cms.vstring("hltEG200HEFilter"),
                                      }
...
###################################################################
importTestFiles = 'from EgammaAnalysis.TnPTreeProducer.etc.tnpInputTestFiles_cff import files%s_%s as inputs' % (
...
###################################################################
process.cand_sequence = cms.Sequence(
    process.init_sequence + process.tag_sequence)
...
process.tnpEleReco = cms.EDAnalyzer("TagProbeFitTreeProducer",
                                    ...
                                    flags=cms.PSet(
                                        passingRECO=cms.InputTag(
                                            "probeSCEle", "superclusters"),
                                        passingRECOEcalDriven=cms.InputTag(
                                            "probeSCEle", "superclustersEcalDriven"),
                                        passingRECOTrackDriven=cms.InputTag(
                                            "probeSCEle", "superclustersTrackDriven")
                                    ),
...
for probeEleModule in str(process.ele_sequence).split('+'):
    if not 'probeEle' in probeEleModule or probeEleModule in ['probeEle', 'probeEleL1matched']:
        continue
    setattr(process.tnpEleTrig.flags, probeEleModule.replace(
        'probeEle', 'passing'), cms.InputTag(probeEleModule))
    setattr(process.tnpEleIDs.flags,  probeEleModule.replace(
        'probeEle', 'passing'), cms.InputTag(probeEleModule))
...
for probePhoModule in str(process.pho_sequence).split('+'):
    if not 'probePho' in probePhoModule or probePhoModule == 'probePho':
        continue
    setattr(process.tnpPhoIDs.flags, probePhoModule.replace(
        'probePho', 'passing'), cms.InputTag(probePhoModule))


# Add SUSY variables to the "variables", add SUSY IDs to the "flags" [kind of deprecated, better ways to add these]
if options['addSUSY']:
    setattr(process.tnpEleIDs.variables, 'el_miniIsoChg',
            cms.string("userFloat('miniIsoChg')"))
    setattr(process.tnpEleIDs.variables, 'el_miniIsoAll',
            cms.string("userFloat('miniIsoAll')"))
    setattr(process.tnpEleIDs.variables, 'el_ptRatio',
            cms.string("userFloat('ptRatio')"))
    setattr(process.tnpEleIDs.variables, 'el_ptRatioUncorr',
            cms.string("userFloat('ptRatioUncorr')"))
    setattr(process.tnpEleIDs.variables, 'el_ptRel',
            cms.string("userFloat('ptRel')"))
    setattr(process.tnpEleIDs.variables, 'el_MVATTH',
            cms.InputTag("susyEleVarHelper:electronMVATTH"))
    setattr(process.tnpEleIDs.variables, 'el_sip3d',
            cms.InputTag("susyEleVarHelper:sip3d"))
