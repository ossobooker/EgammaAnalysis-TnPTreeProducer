...
EleProbeVariablesToStore = cms.PSet(
    ...
    el_sc_et=cms.string(
        "superCluster().energy*sin(superClusterPosition.theta)"),
    ...
    el_dEtaSeed=cms.string(
        "deltaEtaSuperClusterTrackAtVtx+log(tan(superCluster.position.theta/2))-log(tan(superCluster.seed.position.theta/2))"),
    ...
    # this one seem to be always 0 in CMSSW_10_2_X
    ...
    el_relIso03_dB=cms.string(
        "(pfIsolationVariables().sumChargedHadronPt + max(pfIsolationVariables().sumNeutralHadronEt + pfIsolationVariables().sumPhotonEt - 0.5 * pfIsolationVariables().sumPUPt,0.0)) / pt() "),
...
PhoProbeVariablesToStore = cms.PSet(
    ...
    ph_sc_et=cms.string(
        "superCluster.energy*sin(superCluster.position.theta)"),
    ...
    ph_preshower_energy_plane1=cms.string(
        "superCluster.preshowerEnergyPlane1"),
    ph_preshower_energy_plane2=cms.string(
        "superCluster.preshowerEnergyPlane2"),
...
TagVariablesToStore = cms.PSet(
    ...
    el_sip=cms.InputTag("eleVarHelper:sip"),
    Ele_nonTrigMVA80X=cms.InputTag(
        "electronMVAValueMapProducer:ElectronMVAEstimatorRun2Spring16GeneralPurposeV1Values"),
    Ele_hzzMVA80X=cms.InputTag(
        "electronMVAValueMapProducer:ElectronMVAEstimatorRun2Spring16HZZV1Values"),

    Ele_trigMVA=cms.InputTag(
        "electronMVAValueMapProducer:ElectronMVAEstimatorRun2Fall17IsoV1Values"),

    Ele_noIsoMVA94X=cms.InputTag(
        "electronMVAValueMapProducer:ElectronMVAEstimatorRun2Fall17NoIsoV1Values"),
    Ele_IsoMVA94X=cms.InputTag(
        "electronMVAValueMapProducer:ElectronMVAEstimatorRun2Fall17IsoV1Values"),
    Ele_noIsoMVA94XV2=cms.InputTag(
        "electronMVAValueMapProducer:ElectronMVAEstimatorRun2Fall17NoIsoV2Values"),
    Ele_IsoMVA94XV2=cms.InputTag(
...
def setupTnPVariablesForAOD():
    CommonStuffForSuperClusterProbe.pileupInfoTag = cms.InputTag(
        "addPileupInfo")
    CommonStuffForSuperClusterProbe.vertexCollection = cms.InputTag(
        "offlinePrimaryVerticesWithBS")
    CommonStuffForSuperClusterProbe.pfMet = cms.InputTag("pfMet")

    CommonStuffForGsfElectronProbe.pileupInfoTag = cms.InputTag(
        "addPileupInfo")
    CommonStuffForGsfElectronProbe.vertexCollection = cms.InputTag(
        "offlinePrimaryVerticesWithBS")
    ...
    CommonStuffForPhotonProbe.vertexCollection = cms.InputTag(
        "offlinePrimaryVerticesWithBS")
